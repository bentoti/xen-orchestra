...
class AuditXoPlugin {
  ...
  async _storeLastFingerprintInCustomerDb() {
    ...
    let errorOnIntegrityCheck = false
    try {
      await Promise.all(
        hashes.map(async (hash, key) => {
          if (key + 1 === hashes.length - 1) {
            return
          }
          await this.checkIntegrity(hash, hashes[key + 1])
        })
      )
    } catch (error) {
      errorOnIntegrityCheck = true
    }

    const { oldest, newest, error } = await this._generateFingerprint({
      oldest: hashes[hashes.length - 1],
    })

    if (errorOnIntegrityCheck || error !== undefined) {
    ...
    await xo.audit.extendChain(oldest, newest)
  }
